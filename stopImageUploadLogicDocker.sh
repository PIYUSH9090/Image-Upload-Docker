...
docker container ls -f ancestor=$DOCKERUSER_ID/image-upload-logic
echo "Stopping image-upload-logic container"
docker container stop $(docker container ls -f ancestor=$DOCKERUSER_ID/image-upload-logic -aq)
...
docker container rm $(docker container ls -f ancestor=$DOCKERUSER_ID/image-upload-logic -aq)
echo "List of containers running now"
docker container ls -a